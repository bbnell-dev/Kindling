...
function selectedDisplayMode() {
  return modeSelect?.value || readPreferences().mode || "light";
}
...
function normalizeEntry(entry) {
  ...
  const normalized = {
    ...
    pain: Number.isFinite(Number(entry?.pain)) ? Number(entry.pain) : 5,
    capacity: Number.isFinite(Number(entry?.capacity)) ? Number(entry.capacity) : 5,
    locations: Array.isArray(entry?.locations) ? entry.locations : [],
    locationDetails: entry?.locationDetails && typeof entry.locationDetails === "object" ? entry.locationDetails : {},
    context: entry?.context || "",
    helped: entry?.helped || "",
    worsened: entry?.worsened || "",
    note: entry?.note || ""
  };

  if (normalized.type === "missed") {
    normalized.pain = null;
...
function readEntries() {
  try {
    ...
    if (scoped) {
      const entries = JSON.parse(scoped) || [];
...
function kindlingStorageKeys() {
  return Object.keys(localStorage).filter((key) => key.startsWith("kindling-"));
}
...
function exportBackup() {
  const backup = {
    app: "Kindling Journal",
    version: 1,
    exportedAt: new Date().toISOString(),
    activeProfileId: getActiveProfileId(),
    data: Object.fromEntries(kindlingStorageKeys().map((key) => [key, localStorage.getItem(key)]))
  };
  const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" });
  const link = document.createElement("a");
  const date = new Date().toISOString().slice(0, 10);
  link.href = URL.createObjectURL(blob);
  link.download = `kindling-journal-backup-${date}.json`;
...
function restoreBackupPayload(backup) {
  if (!backup || backup.app !== "Kindling Journal" || !backup.data || typeof backup.data !== "object") {
    throw new Error("This does not look like a Kindling backup file.");
  }

  kindlingStorageKeys().forEach((key) => localStorage.removeItem(key));
  Object.entries(backup.data).forEach(([key, value]) => {
    if (key.startsWith("kindling-") && typeof value === "string") {
...
function importBackup(file) {
  ...
  reader.addEventListener("load", () => {
    try {
      ...
      const confirmed = window.confirm("Importing this backup will replace the saved Kindling data in this browser. Continue?");
      if (!confirmed) {
      ...
      setBackupStatus(error.message || "Backup could not be imported.");
    } finally {
...
function applyPreferences() {
  ...
  const mode = ["light", "dark", "system"].includes(preferences.mode) ? preferences.mode : "light";
  const resolvedMode = mode === "system" && devicePrefersDark()
    ? "dark"
    : mode === "dark"
      ? "dark"
      : "light";
...
listen(painInput, "input", updateRanges);
listen(capacityInput, "input", updateRanges);
listen(byId("new-prompt"), "click", rotatePrompt);
listen(byId("add-location"), "click", addCustomLocation);
listen(editLocationsButton, "click", () => {
  setFocusAreasEditing(!focusAreasEditing);
});
listen(byId("add-positive-impact"), "click", () => {
...
listen(byId("add-negative-impact"), "click", () => {
...
listen(customLocationInput, "keydown", (event) => {
  if (event.key === "Enter") {
    event.preventDefault();
    addCustomLocation();
  }
});
listen(customPositiveImpactInput, "keydown", (event) => {
  if (event.key === "Enter") {
    event.preventDefault();
    addCustomImpact(customPositiveImpactInput, customPositiveImpactsKey, "helped");
  }
});
listen(customNegativeImpactInput, "keydown", (event) => {
  if (event.key === "Enter") {
    event.preventDefault();
    addCustomImpact(customNegativeImpactInput, customNegativeImpactsKey, "worsened");
  }
...
listen(byId("location-chips"), "click", (event) => {
...
listen(byId("location-chips"), "change", renderPainDetailFields);
listen(entriesList, "click", (event) => {
...
listen(profileNameInput, "input", () => {
  updatePreference("profileName", profileNameInput.value.trim() || "Kindling");
  syncActiveProfile();
});
listen(avatarSelect, "change", () => {
  updatePreference("avatar", avatarSelect.value);
  syncActiveProfile();
});
listen(profileSelect, "change", () => switchProfile(profileSelect.value));
listen(newProfileButton, "click", createNewProfile);
listen(drinkSelect, "change", () => updatePreference("drink", drinkSelect.value));
listen(themeSelect, "change", () => updatePreference("theme", themeSelect.value));
listen(modeSelect, "change", () => updatePreference("mode", selectedDisplayMode()));
listen(exportBackupButton, "click", exportBackup);
listen(importBackupInput, "change", () => importBackup(importBackupInput.files?.[0]));
const colorSchemeQuery = window.matchMedia?.("(prefers-color-scheme: dark)");
colorSchemeQuery?.addEventListener?.("change", applyPreferences);
window.addEventListener("hashchange", routeToHash);

listen(byId("clear-sample"), "click", () => {
...
listen(byId("dismiss-missed"), "click", () => {
...
listen(missedForm, "submit", (event) => {
  event.preventDefault();
